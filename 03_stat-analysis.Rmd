...
```
Creation de variables et d'un environnement de travail(data.frame):
```{r}
...
Construction d'un objet phyloseq à partir des résultats de dada2: 
```{r}
...
ps
```
On voit ci-dessus l'objet ps
On utilise maintenant cet objet pour travailler avec phyloseq et faire des études sur les données: 
On commence par l'alpha-diversité en traçant les graphiques d'alpha diversité en fonction de l'indice de Shannon dans un premier temps, et de Simpson deuxièmement. On distingue deux temps qui sont regroupés par un changement de couleur. 
```{r}
plot_richness(ps, x="Day", measures=c("Shannon", "Simpson"), color="When")
```
On continue par effectuer une ordination pour l'étude par groupes:
```{r}
...
Et on trace le graphique des résultats de l'ordination:
```{r}
plot_ordination(ps.prop, ord.nmds.bray, color="When", title="Bray NMDS")
```

Finalement on effectue un graphique de barres pour observer l'abondance relative des espèces en fonction de leur groupe (Early ou late). On choisi les 20 plus abondantes:
```{r}
...
```
An useful next step is to explore feature prevalence in the dataset, which we will define here as the number of samples in which a taxon appears at least once.

On enleve les NA qui sont uncharacterized:
```{r}
